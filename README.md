...
- **Wake word** — turn it on in **Settings → Dictation** and set your own
  ...
  - **Wake word before each request** — a toggle right on the voice screen.
    When on, after every request it *soft-mutes*: it keeps playing replies and
    can still be summoned, but won't take another instruction until it hears the
    phrase again, so a room full of side-conversation never becomes a command.
    It defaults to **on when you opened the session by wake word** (you're
    hands-free) and **off when you opened it by clicking** (you're at the
    machine) — flip it live either way. The gear icon on the voice screen opens
    the full app settings without leaving voice mode.
